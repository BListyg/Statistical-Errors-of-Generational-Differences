...
#What if outcomes & continuous age are skewed towards older people?
...
title(main = "Age skewed towards *Older* Respondents, Negatively related w/ Entitlement", sub = "R^2")

axis(side = 1, at = seq(0,80,10), label = seq(0.1,0.9,.1))
...
d <- replicate(50,t(apply(matrix(seq(0.01,0.99,.01)), 1, FUN = agesim, y=300))[,4])

e <- replicate(50,t(apply(matrix(seq(0.01,0.99,.01)), 1, FUN = agesim, y=300))[,5])

f <- replicate(50,t(apply(matrix(seq(0.01,0.99,.01)), 1, FUN = agesim, y=300))[,6])
